...
{
    public int PlayerOnePosition = 1;
    public Vector3 CurrentPiecePosition;
    private Dictionary<string, Vector3> gamePieceLocation = new Dictionary<string, Vector3>
    {
        {"Current Piece Location", new Vector3(1.0f, 0.5f, 0.0f)},
    };
    private Dictionary<string, Vector3> positions = new Dictionary<string, Vector3>
    {
        {"Position 1", new Vector3(1.0f, 0.5f, 0.0f)},
    ...
    private void Start()
    {
        ...
        GameObject myObject = GameObject.Find("Player 1 Piece");
        Debug.Log("Coordinates of Player 1 Piece: " + myObject);

    }

    public int UpdatePlayerOnePosition()
    {
        System.Random random = new System.Random();
        bool End = false;
        if (!End)
        {
            ...
            {
                int i = 0;
                int diceTotal = random.Next(1, 13);
                while (i < diceTotal)
                {
                    PlayerOnePosition++;
                    i++;
        ...
        Debug.Log("Position: " + PlayerOnePosition);
        return PlayerOnePosition;
    }
    public void MovePlayerOnePiece()
    {
        GameObject PlayerOnePiece = GameObject.Find("Player 1 Piece");
        Debug.Log(""+PlayerOnePiece);
        Vector3 CurrentPiecePosition = positions["Position " + PlayerOnePosition];
        Debug.Log(""+CurrentPiecePosition);
        PlayerOnePiece.transform.position = CurrentPiecePosition;
        gamePieceLocation["Current Piece Location"] = CurrentPiecePosition;//y
    }
}
